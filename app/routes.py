...
from app.forms import LoginForm, RegistrationForm, EditProfileForm, RegisterAssetForm
...
def register_asset():
    ...
    if form.validate_on_submit():
        asset = Asset(
            ...
            asset_opinion=form.asset_opinion.data
            )
        db.session.add(asset)
        db.session.commit()
        flash('Congratulations, you have created an asset!')
        return redirect(url_for('index'))
    return render_template('register_asset.html', title='Register Asset', form=form)
